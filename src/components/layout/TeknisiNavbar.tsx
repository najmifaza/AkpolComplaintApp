...
export default function TeknisiNavbar({ profile }: { profile: { nama?: string; nrp?: string; role?: string } }) {
        ...
        <div className="flex justify-between h-20">
          <div className="flex items-center">
            <div className="shrink-0 flex items-center">
              ...
              <Link href="/teknisi/dashboard" className="text-xl sm:text-2xl font-black tracking-tight text-white uppercase drop-shadow-sm flex items-center">
                DUMAS <span className="text-amber-400 mx-1">AKPOL</span>
                <span className="hidden sm:inline-block ml-3 text-xs bg-amber-500 text-slate-900 px-2 py-0.5 rounded-sm font-bold align-middle">TEKNISI</span>
              </Link>
            </div>
          </div>
          ...
          <div className="flex items-center md:hidden">
            <span className="mr-3 text-[10px] bg-amber-500 text-slate-900 px-2 py-0.5 rounded-sm font-bold sm:hidden">TEKNISI</span>
            <button
            ...
            <div className="flex items-center px-5">
              <div className="shrink-0">
                <div className="h-10 w-10 rounded-full bg-slate-700 flex items-center justify-center text-amber-400 font-bold border border-slate-600">
                  {profile?.nama?.charAt(0).toUpperCase() || 'T'}
                </div>
              </div>
              <div className="ml-3">
                <div className="text-base font-bold text-white leading-none">{profile?.nama || 'Teknisi'}</div>
